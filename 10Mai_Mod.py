...
GOOGLE_CLOUD_SCAN_PROGRESS_FILE = "googlecloud_scan_progress.txt"
# ==============================================
...
# ========== Classe utilitaire pour rotation de proxies ==========
class ProxyRotator:
    ...
    def current_proxy_info(self):
        """Retourne une description du proxy actif"""
...
class AlphaUnixScanner:
    def __init__(self, max_workers=100, timeout=10):
        ...
        self.proxy_rotation_delay = 1  # Délai entre les requêtes
        self.proxy_manager = None
        self.proxy_rotator = None  # Gestionnaire de rotation intelligente

    def attach_proxy_manager(self, proxy_manager, reuse_limit=5):
        self.proxy_manager = proxy_manager
        self.proxy_rotator = ProxyRotator(proxy_manager, reuse_limit)

    def scan_domain(self, domain):
        """Scan un domaine avec rotation intelligente des proxies"""
    ...
    def scan_file(self, filename, batch_size=100):
        """Scan depuis un fichier de domaines avec workers et rotation proxy"""
        print(Fore.GREEN + f"\n[⚡] Scan turbo activé (workers: {self.max_workers})")
            ...
            try:
                ...
                with self.lock:
                    ...
                    if self.processed % batch_size == 0:
                        print(f"\r⏳ {self.processed}/{total} ({self.processed/total:.1%})", end='')

        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(_scan, domain): domain for domain in domains}
            for future in as_completed(futures):
                res = future.result()
                if res:
                    self.results.append(res)

        print(f"\r✅ Scan complet! {len(self.results)}/{total} actifs")
        return sorted(self.results, key=lambda x: x.get('latency', 9999))
...
def main_menu():
    ...
    print(Fore.CYAN + " 21. 🌐 Scanner via AlphaUnix (proxy rotatif intelligent)")
    ...
    while True:
        ...
        if choice == "1":  # Scanner une plage IP
            ...
            if scanner.results["Up"]:
                choice = input("Re-scanner les hôtes actifs ? (o/n) : ").lower().strip()
                if choice == "o":
                    try:
                        repeat = int(input("Combien de fois souhaitez-vous re-scanner les hôtes actifs ? (1-10) : "))
                        repeat = max(1, min(repeat, 10))
                    except:
                        repeat = 1
            
                    try:
                        pause_sec = int(input("Temps de pause entre chaque re-scan (en secondes, 0-30) : "))
                        pause_sec = max(0, min(pause_sec, 30))
                    except:
                        pause_sec = 0
                    ...
                    for i in range(repeat):
                        print(Fore.CYAN + f"\n🔁 Re-scan {i+1}/{repeat} en cours...")
                        scanner.rescan_active_hosts()
                        print(Fore.GREEN + f"✅ {len(scanner.active_hosts)} hôtes actifs détectés sur cette passe.")
                        if i < repeat - 1 and pause_sec > 0:
                            print(Fore.YELLOW + f"⏳ Pause de {pause_sec} seconde(s)...")
