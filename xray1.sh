...
# Menu de sélection du protocole
select_protocol() {
    ...
    while true; do
        read -p "Choisissez un protocole [1-${#PROTOCOLS[@]}]: " choice
    ...
    echo "$selected_protocol"  # Retirer la conversion en minuscules
}
...
complete_installation() {
    ...
    echo -e "Protocole choisi: ${YELLOW}$protocol${NC}"
    echo -e "Port: ${YELLOW}$port${NC}"
    echo -e "Transport: ${YELLOW}$transport${NC}"
    echo -e "Mode TLS: ${YELLOW}$tls_mode${NC}"
    ...
    if [ "$protocol" == "trojan" ]; then
        read -p "Mot de passe Trojan (laissez vide pour générer aléatoirement): " password
        password=${password:-$(generate_password)}
    else
        read -p "UUID (laissez vide pour générer aléatoirement): " uuid
    ...
    echo -e "${CYAN}Fichier de configuration: ${YELLOW}$CONFIG_FILE${NC}"
