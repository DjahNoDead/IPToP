...
LAUNCHER_URL = "https://raw.githubusercontent.com/DjahNoDead/IPToP/main/LauncherScanner1.py"
...
def main():
    ...
    try:
        # Initialisation
        if not getattr(main, "_init", False):
            display_banner()
            main._init = True
            time.sleep(1)
        ...
        # Vérification des mises à jour du script
...
if __name__ == "__main__":
    # Mode recovery amélioré
    if os.environ.get("IPT_NO_RESTART") != "1":
        if update_self_if_needed():  # Si mise à jour effectuée
            os.environ["IPT_NO_RESTART"] = "1"
            os.execv(sys.executable, [sys.executable] + sys.argv)
    
    # Appel direct sans tentative d'import
    main()
    
if __name__ == "__main__":
    if not update_self_if_needed():
        main()  # Lance votre programme normalement