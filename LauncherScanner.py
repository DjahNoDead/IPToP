...
def update_self_if_needed():
    if "--force-update" in sys.argv:
        print("[🔧] Forçage de la vérification de mise à jour...")
        # Nettoie tous les verrous
        for lock_file in [".update_lock", ".update_done", ".initialized"]:
            if os.path.exists(os.path.join(CACHE_DIR, lock_file)):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--force-update", help="Force la mise à jour du launcher", action="store_true")
    args = parser.parse_args()

    if args.force_update:
        print("[⚡] Mode forcé activé")
        os.environ["IPT_UPDATE_DONE"] = "0"  # Réinitialise le flag
        if os.path.exists(os.path.join(CACHE_DIR, ".update_done")):
            os.remove(os.path.join(CACHE_DIR, ".update_done"))
    
    if os.environ.get("IPT_UPDATE_DONE") != "1" or args.force_update:
        if update_self_if_needed() and not args.force_update:
            os.execv(sys.executable, [sys.executable] + sys.argv)
