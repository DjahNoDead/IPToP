...
class UpdateManager:
    """Gestionnaire de mises à jour intelligent"""
        ...
        try:
            ...
            if online and not is_restart and not install_already_done:
                ...
                try:
                    ...
                    self.logger.debug(f"Impossible de créer le flag: {e}")
            ...
            if online and not is_restart and self.acquire_lock():
                try:
                    has_update, new_version = self.update_manager.check_launcher_update(self.VERSION)
                    
                    if has_update:
                        self.logger.info(f"Mise à jour du launcher vers v{new_version}")
                        if self.update_manager.update_launcher():
                            os.environ["LAUNCHER_RESTART"] = "1"
                            self.logger.info("Redémarrage...")
                            time.sleep(1)
