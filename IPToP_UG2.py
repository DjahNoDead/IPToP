...
class UserManager:
    ...
    def interactive_create_user(self):
        ...
        if confirm:
            if self.create_user(username, password, is_admin, ssh_key, dockerized):
    ...
    def interactive_modify_user(self):
        ...
        choice = input(f"\n{COLORS['bold']}Votre choix (1-4):{COLORS['end']} ").strip()
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            choice = input(f"\n{COLORS['blue']}Votre choix (1-7):{COLORS['end']} ").strip()
    ...
    def create_user(self, username: str, password: Optional[str], is_admin: bool = False, 
                   ssh_key: Optional[str] = None, dockerized: bool = False) -> bool:
    ...
    def list_users(self):
        ...
        print(f"{COLORS['bold']}{'Nom':<15} {'Admin':<6} {'Type':<8} {'SSH Key':<8} {'Créé le'}{COLORS['end']}")
        print("-" * 60)
        
        for username, data in self.user_data.items():
            admin = "✓" if data.get('is_admin', False) else "✗"
            user_type = "Docker" if data.get('dockerized', False) else "Standard"
            ssh_key = "✓" if data.get('has_ssh_key', False) else "✗"
            created_at = data.get('created_at', 'N/A')
            
            if len(created_at) > 10:
                created_at = created_at[:10]  # Afficher seulement la date
            
            print(f"{username:<15} {admin:<6} {user_type:<8} {ssh_key:<8} {created_at}")
