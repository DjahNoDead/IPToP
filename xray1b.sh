...
setup_cdn() {
    ...
    # Nouvelle option : vérification des certificats existants
    echo -e "\n${BLUE}ℹ Options de certificat :${NC}"
    PS3="Choisissez une option : "
    select cert_option in "Générer un nouveau certificat" "Utiliser un certificat existant" "Quitter"; do
        case $REPLY in
            1) break ;;
            2) 
                if get_existing_certificates "$domain"; then
