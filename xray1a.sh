    ...
    if [ "$cdn_provider" == "Cloudflare" ]; then
        ...
        while true; do
            ...
            if echo "$api_response" | grep -q '"success":true'; then
                zone_id=$(echo "$api_response" | jq -r '.result[0].id')
                echo -e "${GREEN}✓ Token validé - Zone ID : $zone_id${NC}"
                
                # Test d'écriture DNS
                test_record="_xray_verify.${domain}"
                if curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records" \
                    -H "Authorization: Bearer $CF_Token" \
                    -H "Content-Type: application/json" \
                    --data "{\"type\":\"TXT\",\"name\":\"$test_record\",\"content\":\"$(date +%s)\",\"ttl\":120}" | grep -q '"success":true'; then
                    
                    # Nettoyage du record test
                    record_id=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$zone_id/dns_records?type=TXT&name=$test_record" \
                    ...
                    echo -e "${RED}⨉ Le token n'a pas les permissions d'écriture DNS!${NC}"
                fi
            else
