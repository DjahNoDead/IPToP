...
import dns.resolver
from bs4 import BeautifulSoup
from bs4 import BeautifulSoup, Comment  # Ajoutez Comment aux imports
import traceback
from urllib.parse import urlparse, urljoin
...
def preparer_proxy_rotatif_si_auto(proxy):
    """Retourne un ProxyRotator prêt à l'emploi si le proxy est 'auto'"""
    if proxy == "auto":
...
class HostScanner:
    ...
    def run(self):
        """Version complète avec gestion correcte de pause/stop pour les fichiers"""
        self._init_controls()
...
def fetch_url_content(url, proxy=None, max_retries=3):
    """Récupère le contenu d'une URL avec gestion robuste des erreurs"""
    timeout_config = (10, 30)  # (connexion, lecture)
    headers = {
