    ...
    case "$protocol" in
        "vless")
            if [ "$transport" = "tcp" ]; then
                echo -e "${BLUE}URL VLESS (Reality):${NC}"
                echo "vless://$uuid@$DOMAIN:$port?type=tcp&security=reality&flow=$flow&sni=$DOMAIN#Xray-VLESS-TCP"
            else
                echo -e "${BLUE}URL VLESS:${NC}"
                echo "vless://$uuid@$DOMAIN:$port?type=$transport&path=${path}&host=$DOMAIN&security=tls#Xray-VLESS-${transport^^}"
            fi
            ;;
        "vmess")
            vmess_config="{
            ...
            echo -e "${BLUE}URL VMess:${NC}"
            echo "vmess://$(echo $vmess_config | base64 -w 0)"
            ;;
        "trojan")
            echo -e "${BLUE}URL Trojan:${NC}"
            echo "trojan://$password@$DOMAIN:$port?type=$transport&path=${path}&host=$DOMAIN&security=tls#Xray-Trojan-${transport^^}"
            ;;
        "shadowsocks")
            echo -e "${BLUE}Configuration Shadowsocks:${NC}"
            echo "Method: $method"
            echo "Password: $password"
            echo "Server: $DOMAIN"
            echo "Port: $port"
            echo "Plugin: v2ray-plugin"
            echo "Plugin Options: tls;host=$DOMAIN;path=$path"
            ;;
    esac
...
# Test de connexion
test_connection() {
    echo -e "${YELLOW}[*] Test de la connexion...${NC}" | tee -a $LOG_FILE
    
    if curl -x socks5://127.0.0.1:1080 -I https://www.google.com --connect-timeout 5 >/dev/null 2>&1; then
        echo -e "${GREEN}[+] Connexion fonctionnelle!${NC}" | tee -a $LOG_FILE
        return 0
    else
        echo -e "${RED}[!] Échec de la connexion${NC}" | tee -a $LOG_FILE
        return 1
    fi
}
